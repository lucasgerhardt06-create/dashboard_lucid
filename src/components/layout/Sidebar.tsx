...
function NavLinks({ compact=false, onClick }: { compact?:boolean; onClick?:()=>void }) {
  ...
  return <>{items.map((item) => { const active = pathname === item.href; return <Link key={item.id} onClick={onClick} href={item.href+(preview?"?apercu=1":"")} className={cn(compact?"grid place-items-center gap-1 rounded-xl px-2 py-1 text-[11px] transition-colors":"flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm transition-colors", active ? (compact?"text-[#CDA1F9] font-bold":"bg-[#B580F4] text-[#1A1224] font-bold") : "text-[#CDC8D6] hover:bg-[#27212C] hover:text-white")}><item.icon className={compact?"size-5 shrink-0":"size-4 shrink-0"} /><span>{compact ? ({matin:"Matin",entonnoir:"Entonnoir",ia:"IA",eleves:"Élèves"}[item.id] ?? item.name) : item.name}</span></Link>; })}</>;
}
...
export function Sidebar() {
  ...
  return <>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-around border-t border-[#332D3D] bg-[#08040A]/80 px-2 pb-[max(env(safe-area-inset-bottom),8px)] pt-2 backdrop-blur-xl backdrop-saturate-150 md:hidden"><NavLinks compact /><button aria-label="Ouvrir le menu" onClick={() => setOpen(true)} className="grid place-items-center gap-1 rounded-xl px-2 text-xs text-[#CDC8D6]"><Menu className="size-5" />Menu</button></div>
